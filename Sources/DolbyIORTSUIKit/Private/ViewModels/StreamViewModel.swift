...
final class StreamViewModel: ObservableObject {
    ...
    @Published var isShowingDetailSingleViewScreen: Bool = false {
        didSet {
            ...
            configurePiP(for: selectedVideoSource)
        }
    }
    ...
    private var internalState: InternalState = .loading {
        didSet {
            ...
            // Configure for PiP, if required
            guard let newlySelectedVideoSource = internalState.selectedVideoSource, let displayMode = internalState.displayMode else {
                stopPiP()
                return
            }
            
            if newlySelectedVideoSource.id != oldValue.selectedVideoSource?.id || displayMode != oldValue.displayMode {
                configurePiP(for: newlySelectedVideoSource)
            }
        }
    }
    ...
    private func startObservers() {
        ...
        Task { @StreamOrchestrator [weak self] in
            guard let self = self else { return }
            let cancellable = await self.streamOrchestrator.statePublisher
                .combineLatest(settingsPublisher)
                .sink { state, settings in
                    Task { @MainActor in
                        switch state {
                        case let .subscribed(sources: sources, numberOfStreamViewers: _):
                            await self.updateState(from: sources, settings: settings)
                        case .connecting, .subscribing, .connected:
                            await self.updateState(.loading)
                        case let .error(streamError):
                            await self.updateState(.error(ErrorViewModel(error: streamError)))
                        case .stopped:
                            await self.updateState(.error(.streamOffline))
                        case .disconnected:
                            await self.updateState(.error(.noInternet))
                        }
                    }
                }
            await self.addCancellable(cancellable)
        }
    }
    
    private func updateState(_ state: InternalState) {
        internalState = state
    }
    ...
    private func configurePiP(for source: StreamSource) {
        guard enablePiP else { return }
        
        guard let viewRendererProvider: ViewRendererProvider = switch self.internalState {
        case let .success(
            displayMode: currentlySelectedDisplayMode,
            sources: _,
            selectedVideoSource: _,
            selectedAudioSource: _,
            settings: _,
            detailSingleStreamViewModel: detailViewModel
        ):
            if let detailViewModel, isShowingDetailSingleViewScreen {
                detailViewModel.viewRendererProvider
            }
            else {
                switch currentlySelectedDisplayMode {
                case let .grid(viewModel):
                    viewModel.viewRendererProvider
                case let .list(viewModel):
                    viewModel.mainViewRendererProvider
        ...
        let viewRenderer = viewRendererProvider
            .renderer(for: source, isPortait: true)
        let targetView = viewRenderer.playbackView
        let pipView = viewRenderer.pipView

        timer?.invalidate()
        timer = nil
        
        timer = Timer.scheduledTimer(withTimeInterval: 0.25, repeats: true) { _ in
            guard targetView.frame.size != .zero else { return }

            self.timer?.invalidate()
            print("---> Configuring PiP on \(source.sourceId.displayLabel), with pipView - \(pipView), on targetView - \(targetView)")
            PiPManager.shared.set(pipView: pipView, with: targetView)
        }
    }
