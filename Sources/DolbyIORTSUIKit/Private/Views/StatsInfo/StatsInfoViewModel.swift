...
final class StatsInfoViewModel: ObservableObject {
    ...
    var data: [StatData] {
        ...
        if let rtt = stats.roundTripTime {
            result.append(StatData(key: String(localized: "stream.stats.rtt.label"), value: String(rtt)))
        }
        if let videoResolution = stats.videoStatsInboundRtp?.videoResolution {
            result.append(StatData(key: String(localized: "stream.stats.video-resolution.label"), value: videoResolution))
        }
        if let fps = stats.videoStatsInboundRtp?.fps {
            result.append(StatData(key: String(localized: "stream.stats.fps.label"), value: String(fps)))
        }
        if let audioBytesReceived = stats.audioStatsInboundRtp?.bytesReceived {
            result.append(StatData(key: String(localized: "stream.stats.audio-total-received.label"), value: formatBytes(bytes: audioBytesReceived)))
        }
        if let videoBytesReceived = stats.videoStatsInboundRtp?.bytesReceived {
            result.append(StatData(key: String(localized: "stream.stats.video-total-received.label"), value: formatBytes(bytes: videoBytesReceived)))
        }
        if let audioPacketsLost = stats.audioStatsInboundRtp?.packetsLost {
            result.append(StatData(key: String(localized: "stream.stats.audio-packet-loss.label"), value: String(audioPacketsLost)))
        }
